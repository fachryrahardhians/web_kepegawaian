        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (old_tim_rangkap_id != 0 && old_bidang_rangkap_id != 0) {
                ...
                if (satker_ids.includes(parseInt(old_tim_rangkap_id))) {
                    ...
                    fetchOptionsWithTim(old_tim_rangkap_id, '#ppk_rangkap', '<?= base_url("api/get/ppk") ?>', old_ppk_rangkap);
                } else {
                    nulifyOption('#ppk_rangkap');
                }

                if (bidang_kpi_id == old_bidang_rangkap_id) {
                    fetchOptions(old_bidang_rangkap_id, '#tim_rangkap', '<?= base_url("api/get/tim") ?>', old_tim_rangkap);
                    fetchOptionsWithTim(0, '#ppk_rangkap', '<?= base_url("api/get/ppk") ?>', old_ppk_rangkap);
                } else {
                    fetchOptions(old_bidang_rangkap_id, '#tim_rangkap', '<?= base_url("api/get/tim") ?>', old_tim_rangkap);
                    nulifyOption('#ppk_rangkap');
                }
            ...
            if (satker_ids.includes(parseInt(old_tim_id))) {
                // jika termasuk satker, kemudian cari ppk
                fetchOptionsWithTim(old_tim_id, '#ppk_utama', '<?= base_url("api/get/tim") ?>', old_ppk);
            } else {
                nulifyOption('#ppk_utama');
            }

            if (bidang_kpi_id == old_bidang_id) {
                fetchOptions(old_bidang_id, '#tim_utama', '<?= base_url("api/get/tim") ?>', old_tim);
                fetchOptionsWithTim(old_tim_id, '#ppk_utama', '<?= base_url("api/get/tim") ?>', old_ppk);
            } else {
                fetchOptions(old_bidang_id, '#tim_utama', '<?= base_url("api/get/tim") ?>', old_tim);
                nulifyOption('#ppk_utama');
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#bidang_utama').on('change', function() {
            const id = $(this).val();
            if (bidang_kpi_id == id) {
                fetchOptions(id, '#tim_utama', '<?= base_url("api/get/tim") ?>', null);
                fetchOptionsWithTim(0, '#ppk_utama', '<?= base_url("api/get/ppk") ?>', null);
            } else {
                fetchOptions(id, '#tim_utama', '<?= base_url("apii/get/tim") ?>', null);
                nulifyOption('#ppk_utama');
            }
        });
        ...
        $('#bidang_rangkap').on('change', function() {
            const id = $(this).val();
            if (bidang_kpi_id == id) {
                fetchOptions(id, '#tim_rangkap', '<?= base_url("api/get/tim") ?>', null);
        ...
        $('#tim_utama').on('change', function() {
            ...
            if (satker_ids.includes(parseInt(id))) {
                // jika termasuk satker, kemudian cari ppk
                fetchOptionsWithTim(id, '#ppk_utama', '<?= base_url("api/get/ppk") ?>', null);
            } else {
                nulifyOption('#ppk_utama');
            }
        ...
        $('#tim_rangkap').on('change', function() {
            ...
            if (satker_ids.includes(parseInt(id))) {
                // jika termasuk satker, kemudian cari ppk
                fetchOptionsWithTim(id, '#ppk_rangkap', '<?= base_url("api/get/ppk") ?>', null);
            } else {
                nulifyOption('#ppk_rangkap');
            }
        });
